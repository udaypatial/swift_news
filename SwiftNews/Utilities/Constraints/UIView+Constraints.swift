...
extension UIView {
    ...
    @discardableResult
    func constrainEdgesVertically(to otherview: UIView, offset: CGFloat, priority: UILayoutPriority = .required, usingSafeArea: Bool = false) -> [NSLayoutConstraint] {
        return constrainEdgesVertically(to: otherview, topInset: offset, bottomInset: offset, usingSafeArea: usingSafeArea)
    }
    
    @discardableResult
    func constrainEdgesVertically(to otherview: UIView, topInset: CGFloat = 0, bottomInset: CGFloat = 0, usingSafeArea: Bool = false) -> [NSLayoutConstraint] {
        var constraints = [NSLayoutConstraint]()
        
        let constrainable = safeConstrainable(for: otherview, usingSafeArea: usingSafeArea)
        constraints.append(
            constrainTop(to: constrainable, nil, offset: topInset)
        )
        constraints.append(
            constrainBottom(to: constrainable, nil, offset: -bottomInset)
        )
        return constraints
    }
    ...
    func constrainAspectRatio(to size: CGSize, relation: ConstraintRelation = .equal, isActive: Bool = true) -> NSLayoutConstraint {
        ...
        case .equal: return widthAnchor.constraint(equalTo: heightAnchor,
                                                    multiplier: size.width/size.height).set(active: isActive)
